...
#Fit with SGD
def fitWithSGD(guess = None, gradient = None, conv_thresh = 0.007, gamma = 0.05, max_iter = 1000000):
   if gradient==None:
   ...
   while it < max_iter :
      grad = gradient(c_sgd)
      res_norm = sum(r**2 for r in grad)**0.5
      if res_norm < conv_thresh or (False and abs(last_res_norm - res_norm) < conv_thresh):
         break
      last_res_norm = res_norm
      c_sgd = c_sgd - gamma * grad
      it += 1
   return it, c_sgd
...
def fitWithCR(guess = None, residual = None, conv_thresh = 0.007, max_iter = 1000000, subspace = None):
   ...
   while it < max_iter:
      ...
      print(res_norm, conv_thresh, res_norm < conv_thresh)
      if res_norm < conv_thresh or (False and abs(last_res_norm - res_norm) < conv_thresh):
         break
      last_res_norm = res_norm
